    ...
    internal static class EvaluationReplayEligibility
    {
        internal static bool SupportsCondition(string condition)
        {
            return condition?.IndexOf(
                ...
                SupportsExpansion(condition);
        }

        internal static bool SupportsPropertyValue(string value) =>
            SupportsExpansion(value);

        private static bool SupportsExpansion(string expression)
        {
            if (string.IsNullOrEmpty(expression))
            {
                return true;
